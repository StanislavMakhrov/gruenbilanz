...
Examples:
  ...
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz list --branch main --limit 1

  # List latest release workflow run
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz list --workflow release.yml --limit 1

  # Watch a specific run (verbose)
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz watch 12345678

  # Watch a specific run (quiet, agent-friendly)
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz watch 12345678 --quiet

  # View run details
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz view 12345678

  # View all logs for a run
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz logs 12345678

  # View logs for a specific step
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz logs 12345678 --step "Build"

  # Trigger release workflow with tag
  scripts/check-workflow-status.sh --repo StanislavMakhrov/gruenbilanz trigger release.yml --field tag=v1.0.0
