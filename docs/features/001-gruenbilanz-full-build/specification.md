...
Erdgas and Heizöl also offer a `MultiInvoiceUpload` panel for uploading multiple invoices; each invoice can have an optional billing month (1–12) and an "Jahresabrechnung" flag.
...
Strom also offers a `MultiInvoiceUpload` panel for uploading multiple electricity invoices; each invoice can have an optional billing month (1–12) and an "Jahresabrechnung" flag.
...
- `metadata` (JSON string; `saveEntry` stores `{ "category": "<EmissionCategory>" }` to enable per-screen filtering in `ScreenChangeLog`)
...
- **Sustainability badge** (`GET /api/badge?format=svg|png|html`): returns the selected format (SVG, PNG, or HTML embed snippet) showing company CO₂e total and year. Dashboard badge button links directly to `?format=svg` for download.
...
- `GET /api/badge?format=svg|png|html` — return badge in the requested format (SVG default).
- `GET /api/profile` — return the singleton `CompanyProfile` (id=1) as JSON; used by the Firmenprofil wizard screen to pre-fill the form on mount.
