...
export default function KpiCard({ totalCO2eKg, employees, year }: KpiCardProps) {
  ...
  return (
    /* White elevated card with accent left border — clear hero metric block */
    <div className="bg-card rounded-2xl border border-border shadow-lg p-6 border-l-4 border-l-primary">
      <h2 className="text-sm font-semibold text-muted-foreground mb-5 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            t CO₂e / MA{' '}
            <span className="text-xs text-muted-foreground/60">({employees} Mitarbeiter)</span>
          </p>
        </div>
      </div>
    </div>
